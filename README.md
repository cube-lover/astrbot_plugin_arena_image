...
- `/竞技场画图模型`：只显示画图模型列表，附带模型创建时间和最近一次上游结果
  - 列表按创建时间从新到旧排，新上线的灰测模型排在最前面；显示的编号和
    `/竞技场切换模型 编号` 用的编号一致
  - 灰测（隐身）模型默认全部放行，不再只有蒙娜丽莎一个；竞技场自己标了「不给用户选」的
    模型仍然不显示，那类在上游就会被拒绝
...
- 厂商为空 = 灰测（隐身）模型，竞技场故意不写，这样你只看到一个代号，猜不出是谁家的。
  这类**默认已经放行**，蒙娜丽莎就是其中之一。只想留正式模型可以在 `docker/.env` 里设
