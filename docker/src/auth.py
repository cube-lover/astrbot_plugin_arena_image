...
def _upsert_browser_session_into_config(config: dict, cookies: list[dict], user_agent: str | None = None) -> bool:
    ...
    if combined and cookie_store.get("arena-auth-prod-v1") != combined:
        ...
        changed = True
    # Promote either a regular or split auth cookie when persistence is enabled.
    # Older versions only promoted split cookies, leaving a valid regular
    # browser cookie invisible to the token selector after a restart.
    auth_cookie = combined or str(cookie_store.get("arena-auth-prod-v1") or "").strip()
    if bool(config.get("persist_arena_auth_cookie")) and auth_cookie:
        existing = config.get("auth_tokens")
        if isinstance(existing, list):
            tokens = [str(t or "").strip() for t in existing if str(t or "").strip()]
        else:
            tokens = []
        # A freshly captured browser cookie is authoritative.  Remove any
        # other Arena session from the pool and make this one the only entry,
        # otherwise a stale token can remain first in round-robin after a
        # restart or a concurrent config write.  Anonymous sign-up sessions
        # are no longer sufficient for Arena LOGIN_GATE, so do not overwrite
        # a real logged-in token with one.
        from .auth import is_logged_in_arena_auth_token  # type: ignore
        if not is_logged_in_arena_auth_token(auth_cookie) and any(
            is_logged_in_arena_auth_token(str(t or "")) for t in existing
        ):
            return changed
        # If the split cookie (.0/.1) is a real login and the direct cookie is
        # only an anonymous session, promote the combined split value.
        if not is_logged_in_arena_auth_token(auth_cookie) and combined:
            try:
                combined_token = combined if combined.startswith("base64-") else f"base64-{combined}"
            except Exception:
                combined_token = ""
            if is_logged_in_arena_auth_token(combined_token):
                auth_cookie = combined_token
        tokens = [auth_cookie]
        if auth_cookie not in tokens:
            tokens.append(auth_cookie)
        if existing != tokens:
...
def _configured_arena_auth_candidates(config: Optional[dict]) -> list[str]:
    """Return unique auth-cookie candidates from every supported config location."""
    ...
    if isinstance(tokens, list):
        for token in tokens:
            add(token)
    add(config.get("auth_token"))

    cookie_store = config.get("browser_cookies")
    if isinstance(cookie_store, dict):
        add(cookie_store.get("arena-auth-prod-v1"))
        split_cookie_records = [
            {"name": name, "value": cookie_store.get(name)}
            for name in ("arena-auth-prod-v1.0", "arena-auth-prod-v1.1")
        ]
        add(_combine_split_arena_auth_cookies(split_cookie_records))

    return candidates


def _expired_base64_auth_candidates(
    ...
    return [
        token
        for token in _configured_arena_auth_candidates(config)
        if token not in excluded
        and token.startswith("base64-")
        and is_arena_auth_token_expired(token, skew_seconds=0)
    ]


def _persist_refreshed_arena_auth_token(
    token: str,
    *,
    old_token: str = "",
    config: Optional[dict] = None,
) -> None:
    """Persist a rotated browser session while preserving the user's other settings."""
    try:
        cfg = config if isinstance(config, dict) else _m().get_config()
        if not isinstance(cfg, dict):
            return
        old_clean = str(old_token or "").strip()
        if old_clean:
            ...
            if isinstance(tokens, list):
                cfg["auth_tokens"] = [
                    value
                    for value in tokens
                    if str(value or "").strip() != old_clean
                ]
            if str(cfg.get("auth_token") or "").strip() == old_clean:
                cfg["auth_token"] = ""
        _upsert_browser_session_into_config(
            cfg,
            [{"name": "arena-auth-prod-v1", "value": str(token).strip()}],
            user_agent=str(cfg.get("user_agent") or "").strip() or None,
        )
        _m().save_config(cfg, preserve_auth_tokens=False)
    except Exception:
        # A refresh remains usable in memory if a transient config write fails.
        pass
...
def _arena_auth_identity_claims(token: str) -> dict:
    session = _decode_arena_auth_session_token(token) or {}
    if not isinstance(session, dict):
        session = {}
    access = str(session.get("access_token") or "").strip()
    jwt = _decode_jwt_payload(access) if access else None
    if not isinstance(jwt, dict):
        jwt = _decode_jwt_payload(token) or {}
    if not isinstance(jwt, dict):
        jwt = {}
    user = session.get("user") if isinstance(session.get("user"), dict) else {}
    supabase_user = (
        session.get("supabaseUser")
        if isinstance(session.get("supabaseUser"), dict)
        else {}
    )
...
def is_logged_in_arena_auth_token(token: str) -> bool:
    """True only for a non-anonymous Arena session that can pass LOGIN_GATE."""
    if not is_probably_valid_arena_auth_token(token):
        return False
    ident = _arena_auth_identity_claims(token)
    jwt = ident["jwt"]
    session = ident["session"]
    user = ident["user"]
    supabase_user = ident["supabase_user"]
    if any(
        _flag_is_true(value)
        for value in (
            jwt.get("is_anonymous"),
            session.get("is_anonymous"),
    ...
    emails = (
        ...
        ident["app_meta"].get("provider"),
        ident["app_meta"].get("providers"),
    )
    for item in emails:
...
async def maybe_refresh_expired_auth_tokens_via_lmarena_http(exclude_tokens: Optional[set] = None) -> Optional[str]:
    ...
    A successful rotation is written back to the browser-cookie store so the
    next request and the next process restart use the fresh session.
    """
    excluded = exclude_tokens or set()

    cfg = _m().get_config()
    expired_base64 = _expired_base64_auth_candidates(cfg, excluded)
    ...
    async with ARENA_AUTH_REFRESH_LOCK:
        ...
        cfg = _m().get_config()
        refreshed_candidates = _expired_base64_auth_candidates(cfg, excluded)
        for old in list(refreshed_candidates or expired_base64):
...
async def maybe_refresh_expired_auth_tokens(exclude_tokens: Optional[set] = None) -> Optional[str]:
    """
    Refresh an expired `arena-auth-prod-v1` base64 session and persist the
    rotated browser cookie when the data directory is writable.
...
def get_next_auth_token(exclude_tokens: set = None, *, allow_ephemeral_fallback: bool = True):
    ...
    configured_pool_was_empty = not auth_tokens
    cookie_store = config.get("browser_cookies")
    browser_token = ""
    if isinstance(cookie_store, dict):
        browser_token = str(cookie_store.get("arena-auth-prod-v1") or "").strip()
        # Prefer a real logged-in session: combine split cookies (.0/.1) and
        # use it when the direct cookie is only an anonymous sign-up.
        try:
            split_combined = (
                str(cookie_store.get("arena-auth-prod-v1.0") or "").strip()
                + str(cookie_store.get("arena-auth-prod-v1.1") or "").strip()
            )
        except Exception:
            split_combined = ""
        if split_combined and not is_logged_in_arena_auth_token(browser_token):
            candidate = (
                split_combined
                if split_combined.startswith("base64-")
                else f"base64-{split_combined}"
            )
            if is_logged_in_arena_auth_token(candidate):
                browser_token = candidate
        if browser_token:
            try:
                browser_token_usable = not is_arena_auth_token_expired(
                    browser_token,
                    skew_seconds=0,
                )
            except Exception:
                browser_token_usable = True
            if browser_token_usable and (
                not exclude_tokens or browser_token not in exclude_tokens
            ):
                # A cookie captured from the live server browser is the most
            ...
            if browser_token_usable and browser_token not in auth_tokens:
                auth_tokens.insert(0, browser_token)
