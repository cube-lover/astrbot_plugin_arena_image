...
def _apply_config_defaults(config: dict) -> None:
    ...
    config.setdefault("interactive_browser_cdp_url", constants.INTERACTIVE_BROWSER_CDP_URL)
    config.setdefault("interactive_browser_vnc_url", constants.INTERACTIVE_BROWSER_VNC_URL)
    config.setdefault(
        "interactive_auth_ttl_seconds",
        constants.DEFAULT_INTERACTIVE_AUTH_TTL_SECONDS,
    )
    config.setdefault(
        "interactive_auth_poll_seconds",
        constants.DEFAULT_INTERACTIVE_AUTH_POLL_SECONDS,
    )
...
def save_config(config: dict, *, preserve_auth_tokens: bool = True) -> None:
    ...
    """
    try:
        os.makedirs(os.path.dirname(os.path.abspath(_current_config_file)), exist_ok=True)
...
def save_models(models: list) -> None:
    """Save models to file."""
    try:
        os.makedirs(os.path.dirname(os.path.abspath(constants.MODELS_FILE)), exist_ok=True)
...
def get_default_config() -> dict:
    ...
    return {
        ...
        "interactive_auth_ttl_seconds": constants.DEFAULT_INTERACTIVE_AUTH_TTL_SECONDS,
        "interactive_auth_poll_seconds": constants.DEFAULT_INTERACTIVE_AUTH_POLL_SECONDS,
    }
