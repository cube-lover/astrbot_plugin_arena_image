...
# Set to True only when actively diagnosing a local fixture.  Production
# deployments stay quiet by default so prompts, tokens, and upstream payloads
# are not written to container logs.
DEBUG = os.environ.get("LM_BRIDGE_DEBUG", "").strip().lower() in {
    "1",
    "true",
    "yes",
    "on",
...
PORT = int(os.environ.get("LM_BRIDGE_PORT", "8000"))
...
# Optional connection to the visible browser service used for manual
# Cloudflare/Arena verification.  Deployments can provide these through the
# environment without putting a public browser URL in the source tree.
INTERACTIVE_BROWSER_CDP_URL = os.environ.get("LM_BRIDGE_BROWSER_CDP_URL", "").strip()
INTERACTIVE_BROWSER_VNC_URL = os.environ.get("LM_BRIDGE_BROWSER_VNC_URL", "").strip()
DEFAULT_INTERACTIVE_AUTH_TTL_SECONDS = 900
DEFAULT_INTERACTIVE_AUTH_POLL_SECONDS = 3
