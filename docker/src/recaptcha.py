...
async def get_recaptcha_v3_token_via_interactive_browser(config: dict) -> Optional[str]:
    """Mint a fresh reCAPTCHA v3 token from the dedicated Arena verification browser."""
    cdp_url = str(
    ...
    try:
        async with _CDPWebSocket(cdp_url) as cdp:
            ...
            pages = [
                t for t in infos
                if isinstance(t, dict) and t.get("type") == "page" and "arena.ai" in str(t.get("url") or "")
            ]
            if not pages:
                return None
