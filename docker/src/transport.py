...
async def fetch_lmarena_stream_via_interactive_browser(
    http_method: str,
    url: str,
    ...
    *,
    ...
    """Submit the upstream request from the signed-in server browser (arena-browser).

    This reuses the logged-in Arena tab via CDP so the request carries the real
    session cookies instead of a synthetic token pool.  It is the correct path
    for experimental image checkpoints that Arena only exposes to a live login.
    """
    try:
        from .interactive_auth import _CDPWebSocket
    except Exception:
        return None
    ...
    try:
        ...
        cdp_url = str(
            cfg.get("interactive_browser_cdp_url")
        ...
        async with _CDPWebSocket(cdp_url, timeout=float(timeout_seconds)) as cdp:
            targets = await cdp.command("Target.getTargets")
            pages = [
                target
                for target in targets.get("targetInfos", [])
                if target.get("type") == "page"
                and "arena.ai" in str(target.get("url") or "")
            ]
            if not pages:
                _m().debug_print("⚠️ Interactive browser transport: no arena.ai page.")
                return None
            page = next(
                (p for p in pages if "text/direct" in str(p.get("url") or "")),
                pages[0],
            )
            ...
            mint_expr = (
                "(async () => { if (!window.grecaptcha || !window.grecaptcha.enterprise) "
                "return ''; return new Promise((r) => { "
                "window.grecaptcha.enterprise.ready(async () => { try { "
                f"r(await window.grecaptcha.enterprise.execute({json.dumps(sitekey)}, "
                f"{{action: {json.dumps(action)}}})); "
                "} catch(e) { r(''); } }); }); })()"
            )
            eval_result = await cdp.command(
                "Runtime.evaluate",
                {"expression": mint_expr, "awaitPromise": True, "returnByValue": True},
                session_id=session_id,
            )
            recaptcha_token = (eval_result.get("result", {}).get("value") or "")
            if recaptcha_token:
                body_payload["recaptchaV3Token"] = recaptcha_token

            fetch_expr = (
                "(async () => { try { const r = await fetch("
                + json.dumps(fetch_path)
                + ", { method: "
                + json.dumps(str(http_method or "POST").upper())
                + ", headers: { "
                + "'Content-Type': 'text/plain;charset=UTF-8', "
                + "'Accept': 'text/event-stream' }, "
                + "body: "
                + json.dumps(json.dumps(body_payload))
                + ", credentials: 'include' }); const t = await r.text(); "
                + "const hs = {}; r.headers.forEach((v, k) => { hs[k] = v; }); "
                + "return { status: r.status, headers: hs, text: t }; } "
                + "catch (e) { return { status: 502, headers: {}, text: 'FETCH_ERROR:' + String(e) }; } })()"
            )
            fetch_result = await cdp.command(
                "Runtime.evaluate",
                {"expression": fetch_expr, "awaitPromise": True, "returnByValue": True},
                session_id=session_id,
            )
            value = (fetch_result.get("result", {}).get("value") or {})
            ...
            return httpx.Response(
                ...
                request=httpx.Request(str(http_method).upper() or "POST", str(url or "")),
            )
    except Exception as exc:
        _m().debug_print(f"⚠️ Interactive browser transport failed: {type(exc).__name__}: {exc}")
        return None


async def fetch_via_arena_browser_cdp(url: str, *, method: str = "POST", headers: dict | None = None, body: str = "", timeout_seconds: float = 30.0, config: dict | None = None) -> Optional[dict]:
    """Post raw text to Arena from the arena-browser Chrome tab via CDP.

    Returns dict(status, headers, text) or None. Uses the signed-in browser
    session so Cloudflare/cf_clearance/arena-auth cookies are present.
    """
    try:
        from .interactive_auth import _CDPWebSocket
    except Exception:
    ...
    try:
        cfg = config if isinstance(config, dict) else _m().get_config()
        cdp_url = str(cfg.get("interactive_browser_cdp_url")
                      or os.environ.get("LM_BRIDGE_BROWSER_CDP_URL") or "").strip()
        if not cdp_url:
            return None
        async with _CDPWebSocket(cdp_url, timeout=float(timeout_seconds)) as cdp:
            targets = await cdp.command("Target.getTargets")
            pages = [t for t in targets.get("targetInfos", [])
                     if t.get("type") == "page" and "arena.ai" in str(t.get("url") or "")]
            if not pages:
                return None
            page = next((p for p in pages if "text/direct" in str(p.get("url") or "")), pages[0])
            attached = await cdp.command("Target.attachToTarget",
                                         {"targetId": page["targetId"], "flatten": True})
            sid = str(attached.get("sessionId") or "")
            hdrs = dict(headers or {})
            hdrs.setdefault("Content-Type", "text/plain;charset=UTF-8")
            hdrs.setdefault("Accept", "text/x-component")
            expr = (
                "(async () => { try { const r = await fetch("
                + json.dumps(url)
                + ", { method: " + json.dumps(str(method or "POST").upper())
            ...
            if not isinstance(value, dict):
                return None
            return {"status": int(value.get("status") or 0),
                    "headers": value.get("headers") or {},
                    "text": str(value.get("text") or "")}
    except Exception as exc:
        try:
            from . import main as _main
            _main.debug_print(f"⚠️ CDP browser fetch failed: {type(exc).__name__}: {exc}")
        except Exception:
            pass
        return None
                    ...
                    try:
                        ...
                        if meta is not None:
                            ...
                            if isinstance(res, dict) and not res.get("__streaming"):
                                ...
                                # A streaming fetch may have completed without
                                # reporting a body; retain the successful status
                                # so the caller can consume any queued lines.
